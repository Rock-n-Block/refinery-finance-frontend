  ...
  return (
    <div className="lottery-round box-shadow box-white">
      <div className="lottery-round__title text-md text-med">
        <span className="text-purple">Round</span> <span className="text-orange">#{index}</span>
      ...
      <div className="lottery-round__box lottery-round__numbers">
        <div className="lottery-round__numbers-title text-med text-purple">Winning numbers</div>
      ...
      <div className="lottery-round__box lottery-round__total">
        <div className="lottery-round__total-title text-med text-purple">Total prizes</div>
        <div className="box-f-ai-c">
          <img src={LogoImg} alt="" className="lottery-round__total-img" />
          <div className="h2-lg text-purple text-med lottery-round__total-coins">3,100.5</div>
          <div className="text-med text-purple lottery-round__total-coins-name">CAKE</div>
        </div>
      </div>
      <div className="lottery-round__box lottery-round__statistic">
        ...
        {statistic.matched.map((row) => (
          <div className="lottery-round__statistic-row" key={nextId()}>
            <div className="text-bold text-purple">{row.match}</div>
            <div className="text-bold text-purple text-center">{row.winners}</div>
            <div className="text-bold text-purple">{row.prize}</div>
          </div>
        ))}
        <div className="lottery-round__statistic-burned box-f-ai-c box-f-jc-sb text-bold text-purple">
          <span>Burned</span>
      ...
      {user.address ? (
        <Button className="lottery-round__btn" onClick={handleOpenModal}>
          <span className="text-md text-white text-bold">Show Winning Tikets</span>
        </Button>
      ) : (
        <Button className="lottery-round__btn" onClick={connector.connect}>
          <span className="text-md text-white text-bold">Unlock Wallet</span>
        </Button>
      )}
    </div>
  );
